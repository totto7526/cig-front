...
function RegisterSale() {
  ...
  const callFormaPago = async () => {
    const response = await clienteAxios.get('/api/v1/fomas-pago')
    setListFormaPago(response.data)
  }
  const callModalidad = async () => {
    const response = await clienteAxios.get('/api/v1/modalidades')
    setListModalidad(response.data)
  }

  const callProductos = async () => {
  ...
  const submitCrearVenta = async (e) => {
    ...
    const response = await clienteAxios.get('/api/v1/clientes/cliente', { params: { identificacion:identificacion}})
    ...
    try {
      ...
      await Swal.fire({
        ...
        if (result.isConfirmed) {
          const response = await clienteAxios.post('/api/v1/ventas/crear', register);
        }
      })
      // Mensaje de exito
      Swal.fire({
                  ...
                  <div>
                    ...
                    <div>
                      ...
                      <Button
                        ...
                        variant="contained"
                        onClick={
                          e => submitCrearDetalles(idProducto,cantidad,descuento,justificacion)
                        }
                      >
                        AÑADIR DETALLES
                      </Button>
                        ...
                        <TableBody>
                          {details.map((row) => (
                            ...
                            >
                              ...
                              <TableCell align='left'>{row.justificacion}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
